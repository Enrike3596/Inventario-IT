    ...
    public class ParqueaderosController : ControllerBase
    {
        ...
        public async Task<IActionResult> Crear([FromBody] ParqueaderoCreateDTO dto)
        {
            try
            {
                ...
                return CreatedAtAction(nameof(ObtenerPorId), new { id = data.IdParqueadero }, ResponseHelper.Created(data));
            }
            catch (Exception ex)
            {
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Actualizar(int id, [FromBody] ParqueaderoUpdateDTO dto)
        {
            try
            {
        ...
        public async Task<IActionResult> Eliminar(int id)
        {
            try
            {
                ...
                if (!result)
                    return NotFound(ResponseHelper.NotFound());
                return Ok(ResponseHelper.Success(null, "Parqueadero eliminado exitosamente."));
            }
            catch (Exception ex)
            {
                return StatusCode(500, ResponseHelper.Error(ex.Message));
