...
namespace Controllers
{
    ...
    public class CategoriasActivoController : ControllerBase
    {
        ...
        public CategoriasActivoController(ICategoriaActivoService service)
        {
            _service = service;
        }
        ...
        public async Task<IActionResult> Crear([FromBody] CategoriaActivoCreateDTO dto)
        {
            try
            {
                ...
                return CreatedAtAction(nameof(ObtenerPorId), new { id = data.IdCategoria }, ResponseHelper.Created(data));
            }
            catch (Exception ex)
            {
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Actualizar(int id, [FromBody] CategoriaActivoUpdateDTO dto)
        {
            try
            {
        ...
        public async Task<IActionResult> Eliminar(int id)
        {
            try
            {
                ...
                if (!result)
                    return NotFound(ResponseHelper.NotFound());
                return Ok(ResponseHelper.Success(null, "Categoría eliminada exitosamente."));
            }
            catch (Exception ex)
            {
                return StatusCode(500, ResponseHelper.Error(ex.Message));
