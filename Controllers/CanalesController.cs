...
namespace Controllers
{
    ...
    public class CanalesController : ControllerBase
    {
        ...
        public CanalesController(ICanalService service)
        {
            _service = service;
        }
        ...
        [HttpPost]
        public async Task<IActionResult> Crear([FromBody] CanalCreateDTO dto)
        {
            try
            {
                var data = await _service.CrearAsync(dto);
                return CreatedAtAction(nameof(ObtenerPorId), new { id = data.IdCanal }, ResponseHelper.Created(data));
            }
            catch (Exception ex)
            {
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Actualizar(int id, [FromBody] CanalUpdateDTO dto)
        {
            try
            {
        ...
        public async Task<IActionResult> Eliminar(int id)
        {
            try
            {
                ...
                if (!result)
                    return NotFound(ResponseHelper.NotFound());
                return Ok(ResponseHelper.Success(null, "Canal eliminado exitosamente."));
            }
            catch (Exception ex)
            {
                return StatusCode(500, ResponseHelper.Error(ex.Message));
