    ...
    public class AreaController : ControllerBase
    {
        ...
        public async Task<IActionResult> Crear([FromBody] AreaCreateDTO dto)
        {
            try
            {
                ...
                return CreatedAtAction(nameof(ObtenerPorId), new { id = data.IdArea }, ResponseHelper.Created(data));
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> Eliminar(int id)
        {
            try
            {
                ...
                if (!result)
                    return NotFound(ResponseHelper.NotFound());
                return Ok(ResponseHelper.Success(null, "Área eliminada exitosamente."));
            }
            catch (Exception ex)
            {
                return StatusCode(500, ResponseHelper.Error(ex.Message));
